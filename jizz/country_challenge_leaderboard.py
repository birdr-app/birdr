...
from jizz.birdr_journey_views import get_journey_levels_ordered, get_level_steps_ordered
...
def _linked_players_by_user_id(user_ids: list[int]) -> dict[int, Player]:
    if not user_ids:
        return {}
...
def journey_player_name(
    journey: BirdrJourney,
    *,
    players_by_user: dict[int, Player] | None = None,
) -> str:
    if journey.player_id:
        return sanitize_player_name(journey.player.name)
    if journey.user_id:
        if players_by_user is not None:
            linked = players_by_user.get(journey.user_id)
        else:
...
def journey_leaderboard_row(
    ...
    *,
    ...
    pending_celebration = (
        ...
        and not champion
        and step_count > 0
        and journey.current_step_sequence >= step_count
    )
    ...
    return {
        'player_name': journey_player_name(journey, players_by_user=players_by_user),
...
def country_challenge_leaderboard(
    *,
    ...
    journeys = BirdrJourney.objects.select_related('country', 'user', 'player')
    if country_code:
        journeys = journeys.filter(country_id=country_code)
    journeys = list(journeys)
    players_by_user = _linked_players_by_user_id(
        [journey.user_id for journey in journeys if not journey.player_id and journey.user_id]
    )
    rows = [
        journey_leaderboard_row(
            journey,
            levels=levels,
            request=request,
            players_by_user=players_by_user,
        )
